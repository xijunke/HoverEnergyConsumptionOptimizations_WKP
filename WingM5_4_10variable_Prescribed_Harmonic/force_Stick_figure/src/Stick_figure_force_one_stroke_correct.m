...
psi_sim=wing_kenimatics(:,3);             % 扭转角——单位是rad
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
x=[127.5671,1.4443,2.2757,1.1398,-0.1799,-0.0203]; % 6个变量
% P_asterisk =17.0572;   L =1.0001;    delta =6.5845e-005;          penaltyfun1 =0.1317;  
% penaltyfun2 =0;         AR =3.7643;   Re =146.9455(132.4445);  penaltyfun5 =0; 
f=x(1);    % T=1/f;          % Hz
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure(4)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Angle=[t,psi_sim*180/pi,phi_pres*180/pi,alpha_sim*180/pi];    % size(B)  % (1000*4)     %——mN
% xlswrite('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\optimal\optimal_wing_motion\t_psi_phi_alpha_3T.xlsx',Angle,'sheet1','A1:D1000');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (2) 截取单个稳定的周期——以攻角变化周期为区间
% 采用攻角为90度对应时刻为上下冲程的结束时间点
% [psi_sim_0, locat_0]=min(abs(psi_sim));   % psi_sim_0 =6.2058e-004; locat_0 =3;
% [psi_sim_0, locat_0]=min(abs(psi_sim(1:150,1)));   % psi_sim_0=0.0019;  locat_0=132;
% psi_sim_0*180/pi  %  ans =-0.1076;
[psi_sim_0, locat_0]=min(abs(psi_sim(250:350,1)));  % psi_sim_0=0.0065 ;  locat_0=48;
% psi_sim_0*180/pi          % ans =-0.37037
locat_0=locat_0+250-1;   % locat_0=297;
T=1/f;
t_T=t(locat_0)+T;
...
psi1=psi_sim((locat_0:num),1);               % 扭转角——单位是rad——注意符号+-
...
% F_normal=xlsread('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\optimal\optimal_wing_motion\Forcenormal_3T.xlsx','A1:D1000');
...
F_inert_y=F_normal((locat_0:num),4); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure(2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% F_ytrandown=sign(F_ytran(k_index1)).*F_ytran(k_index1);  % size(F_ytrandown)   % (15*1)     %  10^(-5)*
% F_yadddown=sign(F_yadd(k_index1)).*F_yadd(k_index1);   % size(F_yadddown)   % (15*1)
% F_yrotdown=sign(F_yrot(k_index1)).*F_yrot(k_index1);
% F_inert_ydown=sign(F_inert_y(k_index1)).*F_inert_y(k_index1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 翅拍上冲程——不考虑扭转
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:1:N_down   % N_down=15
    ...
    h2=text(-1.0,2.2,'for optimized wing kinematics');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% F_ytranup=-sign(F_ytran(k_index2)).*F_ytran(k_index2);  % size(F_ytranup)   % (15*1)     %  10^(-5)*
% F_yaddup=-sign(F_yadd(k_index2)).*F_yadd(k_index2);   % size(F_yaddup)   % (15*1)
% F_yrotup=-sign(F_yrot(k_index2)).*F_yrot(k_index2);
% F_inert_yup=-sign(F_inert_y(k_index2)).*F_inert_y(k_index2); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % animation———————— 动画片段程序开始段——(1)
% Nstep=N_up;    % N_up=15; 
% np=1;                     %  调节总的帧数和时间步长
