...
% xlswrite('D:\KXJ\PassiveRot_dynamic_Science_fruitfly\optimal\optimal_wing_motion\Forcenormal_3T.xlsx',B,'sheet1','A1:D1000');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
title('沿着扭转轴x-axis的气动力矩')
grid on
axis([min(t/T),min(t/T)+1,-2.5,2.5])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 绕扭转轴的功率计算
% 平动环量力矩功率  % uW
...
legend('平动力矩','转动力矩','转动阻尼力矩','虚质量力矩')
title('沿着拍打轴z-axis的气动力矩')
grid on
axis([min(t/T),min(t/T)+1,-50,50])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 拍打轴的功率
% % 平动环量力矩功率  % uW
...
legend('沿着扭转轴惯性功率','沿着拍打轴惯性功率','惯性总功率')
title('沿着扭转轴x_{rw}-axis和拍打轴z_{rr}-axis的惯性功率')
grid on
...
legend('P_{x,trans}','P_{x,rot}','P_{x,rd}','P_{x,add}','P_{x,aero}','P_{x,inertia}','P_x','\itP_{x,total,posi}')
title('沿着扭转轴x_{rw}-axis的气动功率和惯性功率')
grid on
axis([min(t/T),min(t/T)+1,-inf,inf])
%%%%%%%%%%%%%%%%%%%%%%%%%
figure(107)
...
legend('P_{z,trans}','P_{z,rot}','P_{z,rd}','P_{z,add}','P_{z,aero}','P_{z,inertia}','P_z','\itP_{z,total,posi}')
title('沿着拍打轴z_{rr}-axis的气动功率和惯性功率')
grid on
axis([min(t/T),min(t/T)+1,-inf,inf])
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
grid on
axis([min(t/T),min(t/T)+1,0,max(P_total)+0.5])
