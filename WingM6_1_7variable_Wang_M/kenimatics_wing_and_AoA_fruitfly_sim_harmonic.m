function wing_m_output=kenimatics_wing_and_AoA_fruitfly_sim_harmonic(f,phi_m,epsilon,psi_m,zeta,psi_0)
...
% % t_0=t(index)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% psi=pi/2-(eta_m.*tanh(C_eta.*sin(2.*pi.*f.*t+Phi_eta))./tanh(C_eta)+eta_0); % 扭转角、角速度和角加速度——原始公式没有负号;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hold on
t_phi_max=-epsilon/(2.*pi.*f)+T;                             % t_phi_max =-0.0028;
t_psi_0=(asin(-psi_0/psi_m)-zeta)/(2.*pi.*f)+T;        % t_psi_0 =2.4667e-004;
...
% hold on
% plot((t_phi_max+T)/T,phi_m*180/pi,'rs',(t_psi_0+T/2)/T,0,'bs','LineWidth',3)
hold on
delta_t1=t_phi_max+T-(t_psi_0+T/2); % delta_t1 = 8.3364e-004;
delta_t=(pi-((epsilon+asin(-psi_0/psi_m)-zeta)))/(2.*pi.*f); % delta_t =8.3364e-004;
delta=pi-(epsilon+asin(-psi_0/psi_m)-zeta); % delta =0.6682;
delta_deg=delta*180/pi; % delta_deg =38.2840;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hold on
k=0;
t_phi_0=(pi/2+2*k*pi-epsilon)/(2.*pi.*f)+T;  % % t_phi_0 =-8.7945e-004;
psi_specific=psi_m.*sin(2.*pi.*f.*t_phi_0+zeta)+psi_0;  % % psi_specific =-0.9023;
alpha_specific=(pi/2+psi_specific)*180/pi;  % % alpha_specific =38.3039;
...
hold on
t_phi_01=(pi/2+2*k*pi-epsilon)/(2.*pi.*f)+T/2+T;  % % t_phi_01 =0.0030;
psi_specific1=psi_m.*sin(2.*pi.*f.*t_phi_01+zeta)+psi_0;  % % psi_specific1 = 0.8617;
alpha_specific1=(pi/2-psi_specific1)*180/pi;  % % alpha_specific1 =40.6301;
plot(t_phi_01/T,0,'rd',t_phi_01/T,psi_specific1*180/pi,'bd','LineWidth',3)
hold on
