...
eta_m=x(4);
C_eta=x(5);
Phi_eta=x(6);   % eta_0=x(7);
wing_kenimatics=kenimatics_wing_and_AoA_fruitfly_sim(f,phi_m,K,eta_m,C_eta,Phi_eta); %  6个参数的运动学
% wing_kenimatics=kenimatics_wing_and_AoA_fruitfly_sim_Cfcoeff(f,phi_m,K,eta_m,C_eta,Phi_eta,R_wing,C_aver,xr0,C_maxyaxis); 
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% F_yadd1=(-I5y*(domega_z+omega_x.*omega_y)+I6y*domega_x)*10^(-3);  % 早期被采用——有问题
...
C_R=1.55;    % 该系数有问题
k_zrot=1;
% M_zrot=k_zrot*I2z*C_R*omega_x.*V_nonr*10^(-3); % 早期被采用——有问题
