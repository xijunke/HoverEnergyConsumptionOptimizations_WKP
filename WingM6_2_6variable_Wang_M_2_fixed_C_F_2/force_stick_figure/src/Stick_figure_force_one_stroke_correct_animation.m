...
% x =[189.1094,1.1386,0.3733,1.2324,4.8436,-1.5708]; % P_asterisk =9.1074; L = 1.0000;delta =-1.5699e-007; AR =3.7643; Re =171.7340; % 为下一行的精确值 
x =[1.8910943e+002,1.1386312e+0,3.7329401e-001,1.2324038e+0,4.8435708e+0, -1.5707963e+0];
f=x(1);  T=1/f;   % Hz 
phi_m=x(2);
K=x(3);                  %  epsilon=x(3);    
eta_m=x(4);          % phi_0=x(4);       
...
n_t=length(t);
[psi_sim_0, locat_0]=min(abs(psi_sim(1:n_t/4,1)));   % psi_sim_0 =6.2058e-004; locat_0 =3;
T=1/f;
t_T=t(locat_0)+T;
...
for i = 1:1:N_down   % N_down=15
   ...
   h2=text(-2.35,3.25,'for the optimal WKP with 2D translational aerodynamic coefficients');
