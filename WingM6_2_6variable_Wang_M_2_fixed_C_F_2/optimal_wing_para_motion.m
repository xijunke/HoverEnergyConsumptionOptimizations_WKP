...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% x =[209.7713,0.7522,3.0866,1.1282,0.4909,0.0068];
% P_asterisk =10.0143;  L =0.9999;   delta =-7.9186e-005;
% penaltyfun1 =0.1584;  penaltyfun2 =0;    penaltyfun5 =2000;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% final_pop=x;  % options = gaoptimset('InitialPop', final_pop);
% options=gaoptimset('InitialPop', final_pop,'Display','iter','PopulationSize',100,'Generations',100,... 
% options=gaoptimset('Display','iter','PopulationSize',100,'Generations',100,...   % 回复继续搜索，将代数=200修改为代数=5+10
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% x_start=[144.5109,1.0640,0.1209,0.9782,-28.9946,-1.5854];  % 六个变量的翅膀运动学参数——作为初始值
% x_start =[144.6293,1.0884,0.9265,1.2201,-29.2724,-1.5427]; % 六个变量的翅膀运动学参数——作为初始值
% x_start =[188,1.0884,0.9265,1.2201,2.5,-1.5427]; % 六个变量的翅膀运动学参数——作为初始值 % P_asterisk =9.1731; L = 0.8364; delta =-0.1636;
% x_start =[188.0,1.125,0.01,1.2201,5,-1.5427];
% x_start =[169.55,1.1488,0.01,1.0157,2.5,-1.5427]; % P_asterisk =15.6817;L =1.0001;delta =9.2522e-005;
% x_start =[170.2694,1.1581,0.0001,1.0927,4.7195,-1.4926];%P_asterisk=12.4810; L =1.0000; delta =1.0834e-007; %下一行是精确值
x_start =[1.7026941e+002,1.1580537e+0,1.0000010e-004,1.0927357e+0,4.7195217e+0,-1.4925868e+0];
% x_start =[5.8381,0.0010,1.1710,0.4201, 0.4700,-0.4892]; 
options=gaoptimset('Display','iter','PopulationSize',100,'InitialPopulation',x_start,'Generations',50,...   % 继续遗传50代+2400代局部搜索
    'PlotFcns',{@gaplotbestf,@gaplotexpectation,@gaplotstopping,@gaplotbestindiv,@gaplotscores,@gaplotrange},...
    'TolCon',1e-15,'TolFun',1e-15,'UseParallel','always'); %并行计算——可行
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% fminsearchoptions =
% optimset('Display','iter','MaxFunEvals',14000,'TolCon',1e-10,'TolFun',1e-15,'UseParallel','always'); % 可行feasible
% fminsearchoptions=optimset('Display','iter-detailed',...
%                                  'MaxFunEvals',2500,'MaxIter',2500,'TolX',1e-15,'TolFun',1e-15,'UseParallel','always');%初始搜索1
% fminsearchoptions=optimset('Display','iter-detailed',...
%                                  'MaxFunEvals',3500,'MaxIter',3500,'TolX',1e-15,'TolFun',1e-15,'UseParallel','always');%初始搜索2
% fminsearchoptions=optimset('Display','iter-detailed','TolX',1e-15,'TolFun',1e-15,'UseParallel','always'); % 初始搜索3
fminsearchoptions=optimset('Display','iter-detailed','MaxFunEvals',2400,'MaxIter',2400,'TolX',1e-15,'TolFun',1e-15,'UseParallel','always'); % 初始搜索4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%  C_aver=x(2);    % mm
%  xr0=x(3);
% R_wing1=x(1)*10^-3;  %   R_wing=3.004*10^-3;            % m
% C_aver1=x(2)*10^-3;   %  C_aver=0.8854*10^-3;            % m
% xr01=x(3)*10^-3; 
% % f=x(1);               % 
% f=188.7;                  % Hz
% % phi_aver=x(2);   % 
% phi_aver=1.1488;  % rad   % phi_aver*180/pi  % ans =65.8189;
% PHI=2*phi_aver;
...
% xlswrite('D:\cal_result\WingM6_2_6variable_Wang_M_2_fixed_C_F_2\population_scores.xlsx',output_1,'sheet1','A1:G100');
xlswrite('population_scores.xlsx',output_1,'sheet1','A1:G100');
